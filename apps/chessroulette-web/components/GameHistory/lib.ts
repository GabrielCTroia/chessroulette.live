...
export const getNestedChessHistoryIndex = (
  i: ChessHistoryIndex
): ChessHistoryIndex | undefined => (typeof i !== 'number' ? i[2] : undefined);

export const getRootChessHistoryIndex = (i: ChessHistoryIndex): number =>
  isChessRecursiveHistoryIndex(i) ? i[0] : i;
...
export const addMoveToChessHistory = (
  ...
  if (isChessRecursiveHistoryIndex(chessHistoryIndex)) {
    ...
    const nestedBranchIndex = isChessRecursiveHistoryIndex(
  ...
  if (
    ...
    chessHistoryIndex < history.length - 1
  ) {
    const nextHistory = [
...
export const addMoveToChessHistoryAtNextAvailableIndex = (
  ...
  const foundFollowingMoveAndIndex = getAllFollowingMoves(
    history,
    atIndex
  ...
  return addMoveToChessHistory(
    history,
    move,
    getNextAvailableParallelIndex(history, atIndex)
  );
};
...
export const getAlternativeFollowingMoves = (
  ...
  return move.branchedHistories
    .map((branch, branchIndex) => {
      ...
      const reconstructedFollowingMoveIndex = deconstructedAtIndex.reduce(
        (prev, nextIndex) => {
          // First position only
...
export const getAllFollowingMoves = (
  ...
  const alternativeFollowingMoves = getAlternativeFollowingMoves(
    history,
    atIndex
  );

  return [
    ...(mainFollowingMove ? [mainFollowingMove] : []),
    ...alternativeFollowingMoves,
  ];
};

export const incrementChessHistoryIndex = (
  index?: ChessHistoryIndex
): ChessHistoryIndex => {
  if (index === undefined) {
    return 0;
...
export const decrementChessHistoryIndex = (
  index?: ChessHistoryIndex
): ChessHistoryIndex => {
  if (index === undefined) {
    return 0;
...
export const isChessHistoryIndexHigherThan = (
  ...
  normalizeChessHistoryIndex(comparingIndex) >
  normalizeChessHistoryIndex(toIndex);
...
export const normalizeChessHistoryIndex = (
  index: ChessHistoryIndex = 0
): number => {
  if (typeof index === 'number') {
  ...
  const next = Number(
    `${index[0]}${index[1]}${
      flattendBranchIndex > 0 ? flattendBranchIndex : ''
    }`
  );
...
export const incrementChessRecursiveHistoryIndex = (
  ...
  if (isChessRecursiveHistoryIndex(nestedBranchIndex)) {
    return [
      moveIndex,
      branchIndex,
      incrementChessRecursiveHistoryIndex(nestedBranchIndex),
    ];
  }
